...
const focusMvpsListEl = document.querySelector("#grimmaer-mvps-list");
  ...
  games
    ...
    .forEach(game => {
      ...
      const articleLinks = Array.isArray(game.articleLinks) && game.articleLinks.length
        ? game.articleLinks
        : game.articleUrl
          ? [{ title: "Artikel", url: game.articleUrl }]
          : [];
      if (articleLinks.length) {
        articleLinksEl.innerHTML = "";
        articleLinks.forEach((article, index) => {
          const link = document.createElement("a");
          link.className = "card-link article-link";
          link.href = article.url;
          link.target = "_blank";
          link.rel = "noreferrer";
          link.textContent = article.title || (articleLinks.length === 1 ? "Artikel" : `Artikel ${index + 1}`);
          link.title = article.title || link.textContent;
          link.setAttribute("aria-label", article.title || link.textContent);
          articleLinksEl.append(link);
        });
      }

      const videoLink = node.querySelector(".video-link");
      if (game.videoSearch && game.videoUrl) {
        ...
        videoLink.textContent = "YouTube Video";
        videoLink.title = "Kein YouTube-Video vorhanden";
      } else if (game.videoSearchUrl) {
        videoLink.href = game.videoSearchUrl;
        videoLink.classList.remove("disabled");
        videoLink.textContent = "YouTube Suche";
      } else {
        videoLink.href = "https://www.youtube.com/results?search_query=ESA+Grimma+Volleys+2.+Liga+Pro+2025%2F26";
        videoLink.classList.remove("disabled");
        videoLink.textContent = "YouTube Suche";
      }
...
function renderRankChart() {
  ...
  const svg = make("svg", {
    ...
    role: "img",
    "aria-label": "Tabellenplatz von ESA Grimma Volleys nach jedem Spieltag"
  });
